<template>
    <v-app>
        ...
        <v-main class="grey lighten-4">
            <v-container>
                <v-form class="mt-10 py-10 px-10" ref="form" style="background-color: #DED2C4;border-radius: 5px;" v-model="valid" lazy-validation>
                    <v-row>
                        <v-col cols="8">
                    ...
                    <v-row align="center">
                        <v-col class="d-flex align-center">
                            ...
                            ></v-img>
                            <v-btn color="#EAE1D7"
                            @click="openUploadModal('profile_photo')">Upload Image</v-btn>
                        </v-col>
                    </v-row>
                    <v-row>
                        <v-col cols="12" md="6">
                        ...
                        <v-col cols="12" md="6">
                            <v-btn color="primary"
                            @click="openUploadModal('profile_banner')">Add a Profile Banner</v-btn>
                            <v-img
                ...
                <v-form class="mt-10 socialForm py-10 px-10" style="background-color: #DED2C4;border-radius: 5px;" ref="socialForm" v-model="valid" lazy-validation>
                    <v-row>
                        <v-col cols="8">
    ...
    export default {
        ...
        data() {
            ...
            return {
                ...
                form: {
                    ...
                    profile_photo: (userData.profile_photo && userData.profile_photo.trim() !== '' && userData.profile_photo.trim() != null) ? userData.profile_photo : 'https://eu.ui-avatars.com/api/?name=poapedu&size=250',
                    profile_banner: (userData.profile_banner && userData.profile_banner.trim() !== '' && userData.profile_banner.trim() != null) ? userData.profile_banner : 'https://placehold.co/600x400/png?text=poapedu',
                    one_liner_bio: (userData.one_liner_bio && userData.one_liner_bio.trim() !== '') ? userData.one_liner_bio : '',
                    description: (userData.description && userData.description.trim() !== '') ? userData.description : '',
                    userID: userData.learner_id ? userData.learner_id : 1,
                    slug: userData.slug || '',

                },
                socialForm: {
                    linkedin_url: (userData.linkedin_url && userData.linkedin_url.trim() !== '') ? userData.linkedin_url : '',
                    twitter_url: (userData.twitter_url && userData.twitter_url.trim() !== '') ? userData.twitter_url : '',
                    discord_url: (userData.discord_url && userData.discord_url.trim() !== '') ? userData.discord_url : '',
                    github_url: (userData.github_url && userData.github_url.trim() !== '') ? userData.github_url : '',
                    devto_url: (userData.devto_url && userData.devto_url.trim() !== '') ? userData.devto_url : '',
                    website_url: (userData.website_url && userData.website_url.trim() !== '') ? userData.website_url : '',
                    userID: userData.learner_id ? userData.learner_id : 1,
                },
                rules: {
                    required: value => !!value || 'Required.',
                    url: value => /^(ftp|http|https):\/\/[^ "]+$/.test(value) || 'URL must be valid.',
                    linkedin: value => !value || /^(https?:\/\/)?(www\.)?linkedin\.com\/.*$/.test(value) || 'Must be a valid LinkedIn URL.',
                    twitter: value => !value || /^(https?:\/\/)?(www\.)?twitter\.com\/.*$/.test(value) || 'Must be a valid Twitter URL.',
                    discord: value => !value || /^(https?:\/\/)?(www\.)?discord\.com\/.*$/.test(value) || 'Must be a valid Discord URL.',
                    github: value => !value || /^(https?:\/\/)?(www\.)?github\.com\/.*$/.test(value) || 'Must be a valid GitHub URL.',
                    devto: value => !value || /^(https?:\/\/)?(www\.)?dev\.to\/.*$/.test(value) || 'Must be a valid Dev.to URL.',
        ...
        methods: {
            ...
            async checkSlugUniqueness() {
                ...
                try {
                    const response = await fetch(`${process.env.VUE_APP_LOCAL_SERVER_URL}/api/check-slug?slug=${encodeURIComponent(this.form.slug)}`);
                    const data = await response.json();
            ...
            async saveProfile() {
                if (this.$refs.form.validate()) {
                    try {
                    const response = await fetch(`${process.env.VUE_APP_LOCAL_SERVER_URL}/api/save-profile`, {
                        method: 'POST',
                        headers: {
            ...
            async saveSocials() {
                if (this.$refs.socialForm.validate()) {
                    try {
                    const response = await fetch(`${process.env.VUE_APP_LOCAL_SERVER_URL}/api/save-socials`, {
                        method: 'POST',
                        headers: {
                        'Content-Type': 'application/json'
                        },
